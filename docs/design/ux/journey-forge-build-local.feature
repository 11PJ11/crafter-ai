...
Feature: Build nWave Local Candidate
  ...
  Scenario: Version bump from conventional commits
    ...
    And the candidate version should follow format "M.m.p-dev-YYYYMMDD-sequence"
  ...
  Scenario: Force version override
    ...
    And the candidate version should be "2.0.0-dev-${date}-1"
  ...
  Scenario: Daily sequence increments on same day
    Given a candidate "1.3.0-dev-20260201-1" was built earlier today
    When the user runs "forge:build-local-candidate" again
    Then the candidate version should be "1.3.0-dev-20260201-2"
