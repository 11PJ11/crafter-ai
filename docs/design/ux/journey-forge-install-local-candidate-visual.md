...
|----------|-----------------|--------------|------|
| `${candidate_version}` | pyproject.toml [project.version] + branch + date + seq | Steps 2, 4, 5 | HIGH - must match wheel |
| `${version}` | pyproject.toml [project.version] base only | Steps 2, 5 | HIGH - semantic version |
| `${branch}` | git branch name | Steps 2, 5 | LOW - informational |
| `${date}` | Build date YYYYMMDD | Steps 2, 5 | LOW - informational |
| `${sequence}` | Daily build sequence number | Steps 2, 5 | LOW - informational |
...
+---------------------------------------------------------------------------+
|                                                                            |
|  Format: M.m.p-{branch}-{YYYYMMDD}-{daily_sequence}                       |
|                                                                            |
|  Examples:                                                                 |
|    2.1.0-feature-new-agent-20260201-001                                   |
|    2.1.0-master-20260201-003                                              |
|    2.1.0-bugfix-install-20260131-001                                      |
|                                                                            |
|  Components:                                                               |
|    M.m.p          Base semantic version from pyproject.toml               |
|    branch         Git branch name (sanitized)                              |
|    YYYYMMDD       Build date                                               |
|    sequence       Daily build number (001, 002, etc.)                      |
|                                                                            |
|  Notes:                                                                    |
|    - Final release versions (no suffix) only come from CI/CD on main      |
|    - Version bumping happens in forge:build-local, not here               |
|    - This format is PEP 440 compatible (local version identifier)         |
|                                                                            |
+---------------------------------------------------------------------------+
```
