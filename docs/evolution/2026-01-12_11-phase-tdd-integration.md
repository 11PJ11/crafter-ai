...
This project successfully integrated a mandatory 11-phase TDD discipline into the nWave (formerly nWave) framework, transforming TDD from implicit knowledge to explicit, enforceable workflow with comprehensive tracking, validation, and command mapping.
...
**File:** `nWave/templates/step-tdd-cycle-schema.json`
...
**Impact:** Every step file generated by `/nw:split` now includes complete tracking structure.
...
- **Phase Command Mapping:** Explicit mapping of each phase to command invocation
  - Phases 1-6, 10: Internal to `/nw:develop` (main TDD loop)
  - Phase 7: `/nw:review @software-crafter-reviewer implementation`
  - Phase 8: `/nw:refactor --level 4` OR `/nw:mikado` for complex refactoring
  - Phase 9: `/nw:review @software-crafter-reviewer refactored_implementation`
...
**File:** `nWave/agents/software-crafter-reviewer.md`
...
- **Alternative Automation:** `/nw:execute` for fully automated 11-phase execution

**File:** `nWave/tasks/dw/split.md`

**Added Section:** "11-Phase Command Mapping" (30 lines)

**Key Components:**
- **Phase → Command Table:** Shows which command executes which phase
- **Automated Execution:** Documents `/nw:execute` alternative
...
|---|-------|---------|------|----------|------|
| 1 | PREPARE | `/nw:develop` (internal) | Automatic | 3-5 min | G1: Exactly 1 acceptance test active |
| 2 | RED (Acceptance) | `/nw:develop` (internal) | Automatic | 3-5 min | G2: Test fails for valid business reason |
| 3 | RED (Unit) | `/nw:develop` (internal) | Automatic | 5-10 min | G3: Test fails on assertion<br>G4: No mocks inside hexagon |
| 4 | GREEN (Unit) | `/nw:develop` (internal) | Automatic | 10-20 min | Unit test passes |
| 5 | CHECK | `/nw:develop` (internal) | Automatic | 2-3 min | Decision: FAIL→Phase 3, PASS→Phase 6 |
| 6 | GREEN (Acceptance) | `/nw:develop` (internal) | Automatic | 5-10 min | G6: All tests green |
| 7 | REVIEW | `/nw:review` | **Explicit** | 10-15 min | G5: Business language verified |
| 8 | REFACTOR | `/nw:refactor` OR `/nw:mikado` | **Explicit** | 15-30 min | G6: Tests green after each level |
| 9 | POST-REFACTOR REVIEW | `/nw:review` | **Explicit** | 10-15 min | Approval before commit |
| 10 | FINAL VALIDATE | `/nw:develop` (internal) | Automatic | 5-10 min | G7: 100% tests passing |
...
```bash
# Phase 1-6, 10: Automatic (internal to /nw:develop)
/nw:develop order-management --step 01-02

# Phase 7: Explicit REVIEW invocation (MANDATORY)
/nw:review @software-crafter-reviewer implementation \
...
/nw:refactor --level 4 --scope module

# Option B: Complex refactoring with Mikado Method
/nw:mikado --goal "Extract OrderService from OrderController"

# Phase 9: Explicit POST-REFACTOR REVIEW (MANDATORY)
/nw:review @software-crafter-reviewer refactored_implementation \
  "docs/feature/order-management/steps/01-02.json"
...
# - /nw:develop for phases 1-6, 10
# - /nw:review for phases 7, 9
# - /nw:refactor or /nw:mikado for phase 8
...
|------|-------------|------|---------|
| `nWave/templates/step-tdd-cycle-schema.json` | +27 | Template | Add tdd_phase_tracking structure |
| `nWave/agents/software-crafter.md` | +178 | Agent | Add eleven_phase_tdd_methodology |
| `nWave/agents/software-crafter-reviewer.md` | +166 | Agent | Add eleven_phase_validation_protocol |
| `nWave/tasks/dw/develop.md` | +114 | Command | Add 11-Phase Command Invocation Mapping |
| `nWave/tasks/dw/split.md` | +30 | Command | Add 11-Phase Command Mapping |
...
✓ 34 agents installed to ~/.claude/agents/nw/
✓ 20 commands installed to ~/.claude/commands/nw/
...
5. **Multiple Automation Levels:** Manual invocation for learning, `/nw:execute` for efficiency
