...
including the mandatory phase_execution_log with all 8 TDD phases (schema v2.0).
...
from nWave.constants.tdd_phases import REQUIRED_PHASES, PHASE_COUNT
...
def validate_phase_execution_log(
    ...
    """Validate phase_execution_log has all 8 phases properly structured (schema v2.0)."""
    issues: List[str] = []
...
def validate_step_file(file_path: Path) -> Tuple[bool, List[str]]:
    ...
    if "phase_id" in data:
        issues.append(
            ...
            f"Each step contains ALL {PHASE_COUNT} phases, not a single phase."
        )
