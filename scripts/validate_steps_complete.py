...
def validate_all_steps_complete(project_id: str) -> Tuple[bool, List[Dict[str, Any]], str]:
    ...
    if incomplete_steps:
        ...
        for step in incomplete_steps:
            error_lines.append(f"  ❌ {step.get('task_id', step['file'])}: {step['reason']}")
    ...
    return True, [], f"✅ CRITICAL INVARIANT PASSED: All {len(step_files)} steps have COMMIT/PASS"
