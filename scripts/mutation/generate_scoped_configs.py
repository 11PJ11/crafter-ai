#!/usr/bin/env python3
"""
Generate feature-scoped mutation testing configurations from execution-status.yaml

This script extracts implementation files from execution-status.yaml and creates
...
def extract_implementation_files(execution_status_path: str) -> list[str]:
    """Extract all implementation files from execution-status.yaml"""
...
Examples:
  ...
  docs/feature/{project-id}/mutation/cosmic-ray-*.toml
    ...
    execution_status_path = f"docs/feature/{project_id}/execution-status.yaml"
    ...
    print("STEP 1: Extracting implementation files from execution-status.yaml...")
    impl_files = extract_implementation_files(execution_status_path)

    if not impl_files:
        print("❌ No implementation files found in execution-status.yaml")
        sys.exit(1)
