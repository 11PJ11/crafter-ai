...
class VersionParser:
    ...
    def parse_yaml_version(file_path: Path, field: str = "version") -> Optional[str]:
        ...
        try:
            ...
            print(f"Warning: Failed to parse YAML version from {file_path}: {e}",
                  file=sys.stderr)
            return None
    ...
    def parse_markdown_version(file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            print(f"Warning: Failed to parse Markdown version from {file_path}: {e}",
                  file=sys.stderr)
            return None

    @staticmethod
    def parse_version(file_path: Path, format_type: str,
    ...
    def compare_versions(v1: str, v2: str) -> int:
        ...
        try:
            ...
            print(f"Warning: Version comparison failed for {v1} vs {v2}: {e}",
                  file=sys.stderr)
            return 0
...
class GitHelper:
    ...
    @staticmethod
    def get_version_from_head(file_path: str, format_type: str,
                             field: Optional[str] = None) -> Optional[str]:
...
class DocumentationVersionValidator:
    ...
    def _load_dependency_map(self) -> Dict:
        ...
        if not self.dependency_map_path.exists():
            print(f"ERROR: Dependency map not found: {self.dependency_map_path}",
                  file=sys.stderr)
            sys.exit(2)
    ...
    def _get_current_version(self, tracked_file: TrackedFile) -> Optional[str]:
        ...
        ver = VersionParser.parse_version(
            file_path,
            tracked_file.version_format,
            tracked_file.version_field
        )
    ...
    def validate(self) -> bool:
        ...
        for tracked in tracked_files:
            ...
            if not GitHelper.file_has_changes(
                tracked.path,
                validation_rules.get('ignore_whitespace_changes', True)
            ):
            ...
            head_version = GitHelper.get_version_from_head(
                tracked.path,
                tracked.version_format,
                tracked.version_field
            )

            # Validate version format
            if current_version and not VersionParser.validate_version_format(current_version):
        ...
        for tracked in tracked_files:
            ...
            for dependent_config in tracked.triggers_update:
                ...
                dependent_tracked = next(
                    (t for t in tracked_files if t.path == dependent_file),
                    None
                )
                ...
                # Compare versions
                comparison = VersionParser.compare_versions(dependent_version, current_version)
    ...
    def generate_error_report(self) -> Dict:
        ...
        version_not_bumped = [e for e in self.errors if e.error_type == "VERSION_NOT_BUMPED"]
        dependent_outdated = [e for e in self.errors if e.error_type == "DEPENDENT_OUTDATED"]
        ...
        for error in invalid_version:
            resolution_steps.append(f"{step_num}. Fix invalid version format in {error.file}")
            step_num += 1
        ...
        files_to_read = list(set(
            e.file for e in version_not_bumped if e.current_version
        ))
        files_to_edit = list(set(
            e.file for e in version_not_bumped + dependent_outdated
        ))
...
def main():
    ...
    if validator.validate():
        ...
        print("\n" + "="*80, file=sys.stderr)
        print("✗ COMMIT BLOCKED: Documentation version validation failed", file=sys.stderr)
        print("="*80 + "\n", file=sys.stderr)
