...
def validate_output_file(file_path: str, stage: str) -> Tuple[bool, str]:
    ...
    return (
        False,
        f"File '{filename}' not allowed for {stage} stage. Allowed files: {', '.join(allowed_files)}",
    )
...
def check_content_compliance(file_path: str, content: str) -> List[str]:
    ...
    forbidden_patterns = [
        (
            r"execAsync\s*\(",
            "Direct CLI execution detected - use production services instead",
        ),
        (
            r"Process\.Start\s*\(",
            "Direct process execution detected - use production services",
        ),
    ...
    if "test" in file_path.lower() or "acceptance" in file_path.lower():
        ...
        if "GetRequiredService" not in content:
            violations.append(
                "Missing GetRequiredService pattern in test implementation"
            )
        ...
        if technical_count > 0:
            ...
            if ratio < 0.7:
                violations.append(
                    f"Business language ratio too low: {ratio:.1%} (minimum 70%)"
                )

    return violations


def log_output_activity(
    file_path: str, stage: str, status: str, violations: List[str] = None
):
    ...
    try:
        with open(log_file, "a") as f:
            f.write(
                f"[{timestamp}] OUTPUT MONITOR: {status} - {file_path} (stage: {stage})\n"
            )
            if violations:
...
def update_compliance_metrics(file_path: str, violations: List[str]):
    ...
    if metrics["summary"]["total_files"] > 0:
        compliance_rate = (
            metrics["summary"]["compliant_files"] / metrics["summary"]["total_files"]
        )
...
def main():
    ...
    if not is_valid:
        print("❌ CAI OUTPUT VALIDATION ERROR")
    ...
    has_critical = any(
        any(pattern in v for pattern in critical_patterns) for v in violations
    )
