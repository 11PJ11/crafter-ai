"""Tests for troubleshooter.md to TOON v3.0 conversion validation.

Step 03-02 acceptance criteria validation:
- TOON v3.0 file generated: nWave/agents/troubleshooter.toon
- Schema validation passes
- All YAML keys preserved
- Zero syntax errors

Test approach follows software-crafter conversion pattern as reference.
"""
...
# Expected commands from original troubleshooter.md
EXPECTED_COMMANDS = [
    "help",
    "investigate-problem",
...
class TestTOONMetadata:
    ...
    def test_toon_has_agent_id(self, parsed_toon):
        """Agent ID is 'troubleshooter'."""
        assert parsed_toon["id"] == "troubleshooter"
    ...
    def test_toon_has_name_in_metadata(self, parsed_toon):
        """Metadata contains name from ID section role field (Sage | troubleshooter)."""
        assert "name" in parsed_toon["metadata"]
        # Parser extracts name from 'role: Sage | troubleshooter' -> splits on '|', takes first part
        # The parser extracts 'Troubleshooter' from header, but the ID section role 'Sage | troubleshooter'
        # provides more specific name. Either value is acceptable for metadata preservation.
        assert parsed_toon["metadata"]["name"] in ["Sage", "Troubleshooter"]
    ...
    def test_toon_has_role_in_metadata(self, parsed_toon):
        """Metadata contains role with 'troubleshooter'."""
        assert "role" in parsed_toon["metadata"]
        assert "troubleshooter" in parsed_toon["metadata"]["role"]
...
class TestTOONSchemaValidation:
    ...
    def test_compiled_output_has_name(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "troubleshooter" in content.lower()
...
class TestTOONCompleteness:
    """Verify TOON captures key knowledge sections from troubleshooter.md."""
