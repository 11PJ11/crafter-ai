"""Tests for troubleshooter-reviewer roundtrip validation (MD -> TOON -> MD).

Step 04-03 acceptance criteria:
...
This test file validates the troubleshooter-reviewer.toon file specifically,
ensuring the conversion preserves all critical content from the original
troubleshooter-reviewer.md source.
"""
...
@pytest.fixture
def troubleshooter_reviewer_md_exists():
...
@pytest.fixture
def compiled_md_path(troubleshooter_reviewer_toon_exists):
...
@pytest.fixture
def original_content(troubleshooter_reviewer_md_exists):
    """Get original markdown content."""
    return ORIGINAL_MD.read_text()


@pytest.fixture
def toon_content(troubleshooter_reviewer_toon_exists):
    """Get TOON source content."""
    return TOON_FILE.read_text()


class TestTroubleshooterReviewerRoundtripValidation:
    """E2E tests for troubleshooter-reviewer roundtrip validation.

    AC1: MD -> TOON -> MD produces equivalent output
    """

    def test_roundtrip_equivalence_score_threshold(self, compiled_md_path):
        """GIVEN troubleshooter-reviewer.md and compiled output from troubleshooter-reviewer.toon
...
class TestTroubleshooterReviewerYAMLKeyPreservation:
...
class TestTroubleshooterReviewerValueMatching:
    ...
    def test_dependency_tasks_preserved(self, original_content, compiled_content):
        ...
        if original_deps.get("tasks"):
            ...
            # dw/root-why.md is the main task for troubleshooter-reviewer
...
class TestTroubleshooterReviewerEmbeddedKnowledge:
    ...
    def test_embedded_knowledge_markers_in_toon(self, toon_content):
        ...
        # troubleshooter-reviewer.toon has 2 BUILD:INJECT markers
...
class TestTroubleshooterReviewerCriticalSections:
    ...
    def test_compiled_has_name(self, compiled_content):
        ...
        has_name = (
            ...
            "troubleshooter-reviewer" in compiled_content.lower()
        )
...
class TestTroubleshooterReviewerSpecificContent:
    """Test troubleshooter-reviewer-specific content preservation."""
    ...
    def test_review_specialist_identity(self, toon_content):
        """Review specialist identity should be preserved."""
        has_review = "review" in toon_content.lower()
        has_specialist = "specialist" in toon_content.lower()
        assert has_review and has_specialist, "Review specialist identity not preserved"


class TestTroubleshooterReviewerRoundtripIntegrity:
