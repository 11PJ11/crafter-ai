"""Tests for token measurement of troubleshooter-reviewer agent conversion.

Step 04-04 acceptance criteria validation:
...
# File paths
BASELINE_PATH = Path(
    "docs/feature/toon-agent-conversion/measurements/troubleshooter-reviewer-baseline.yaml"
)
COMPARISON_PATH = Path(
    "docs/feature/toon-agent-conversion/measurements/troubleshooter-reviewer-comparison.yaml"
)
MD_SOURCE = Path("nWave/agents/troubleshooter-reviewer.md")
TOON_SOURCE = Path("nWave/agents/troubleshooter-reviewer.toon")
...
class TestBaselineStructure:
    ...
    def test_has_step_id(self, baseline_data):
        ...
        assert baseline_data["step_id"] == "04-04"

    def test_has_agent_name(self, baseline_data):
        """Must have agent name."""
        assert baseline_data["agent"] == "troubleshooter-reviewer"
...
class TestComparisonStructure:
    ...
    def test_has_step_id(self, comparison_data):
        """Must have step identifier."""
        assert comparison_data["step_id"] == "04-04"

    def test_has_agent_name(self, comparison_data):
        """Must have agent name."""
        assert comparison_data["agent"] == "troubleshooter-reviewer"
