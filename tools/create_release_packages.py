...
Creates distributable packages for different IDE platforms from the built framework.
...
class ReleasePackager:
    ...
    def __init__(self, version: str = None, output_dir: Path = None):
        ...
        self.ide_dir = self.dist_dir / "ide"
    ...
    def validate_source(self) -> bool:
        """Validate that source framework exists."""
        if not self.ide_dir.exists():
            self._log(f"IDE bundle not found at: {self.ide_dir}", "ERROR")
            self._log(
                "Run 'python tools/build.py' first to build the framework", "ERROR"
            )
            return False

        agents_dir = self.ide_dir / "agents" / "nw"
        commands_dir = self.ide_dir / "commands" / "nw"

        if not agents_dir.exists() or not commands_dir.exists():
            self._log("Required directories not found in dist/ide/", "ERROR")
            return False
        ...
        if agent_count == 0 or command_count == 0:
            self._log("No agents or commands found in dist/ide/", "ERROR")
            return False
    ...
    def create_package_structure(self, package_name: str) -> Path:
        ...
        # Copy agents
        agents_src = self.ide_dir / "agents" / "nw"
        ...
        # Copy commands
        commands_src = self.ide_dir / "commands" / "nw"
        ...
        scripts_src = self.project_root / "nWave" / "scripts"
        ...
        # Copy templates
        templates_src = self.project_root / "nWave" / "templates"
...
Visit: https://github.com/11PJ11/crafter-ai
...
```bash
curl -O https://github.com/11PJ11/crafter-ai/releases/download/v{self.version}/install-nwave-claude-code.py
python install-nwave-claude-code.py
```
...
- Issues: https://github.com/11PJ11/crafter-ai/issues
- Discussions: https://github.com/11PJ11/crafter-ai/discussions
