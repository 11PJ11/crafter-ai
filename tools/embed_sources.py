...
# Marker patterns
EMBED_START_PATTERN = re.compile(
    r'<!--\s*EMBED_START:([^:]+):(\w+)\s*-->'
)
...
def format_content(content: str, source_path: str, format_type: str) -> Optional[str]:
    ...
    if not formatter:
        ...
        print(f"          Supported formats: {', '.join(formatters.keys())}", file=sys.stderr)
        return None

    return formatter(content, source_path)


def process_file(file_path: str, dry_run: bool = False, verbose: bool = False) -> Tuple[bool, int]:
    """
    ...
    for start_match in starts:
        ...
        end_pattern = re.compile(
            rf'<!--\s*EMBED_END:{re.escape(source_path)}\s*-->'
        )
        end_match = end_pattern.search(remaining_content)

        if not end_match:
            print(f"  [ERROR] No matching EMBED_END for: {source_path}", file=sys.stderr)
            errors += 1
            continue
