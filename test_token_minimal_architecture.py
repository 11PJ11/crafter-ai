...
class MockHook(HookPort):
    def post_execution_hook(self, step_file_path, turn_count, phase_name, execution_status) -> HookResult:
...
def test_orchestrator_context_extraction():
    ...
    try:
        ...
        print(f"✅ Roadmap loaded successfully")
        ...
        # Count total steps
        total_steps = sum(len(phase.get('steps', [])) for phase in roadmap.get('phases', []))
    ...
    try:
        ...
        print(f"✅ Execution status loaded successfully")
    ...
    try:
        ...
        print(f"✅ Task context extracted successfully")
        print(f"   Step ID: {task_context.get('step_id')}")
        print(f"   Name: {task_context.get('name')}")
        print(f"   Acceptance criteria: {len(task_context.get('acceptance_criteria', []))} items")
        ...
        print("   - OLD: Sub-agent loads roadmap: 102k tokens × 15 steps = 1,530k tokens")
    ...
    print("-" * 80)
    required_fields = [
        'step_id', 'name', 'description', 'acceptance_criteria',
        'test_file', 'quality_gates', 'tdd_phases', 'execution_config'
    ]
    ...
    if missing_fields:
        ...
        print(f"✅ All required fields present in extracted context")
        print(f"   Sub-agent can execute without loading roadmap")
        print()
    ...
    expected_phases = [
        "PREPARE", "RED_ACCEPTANCE", "RED_UNIT", "GREEN",
    ...
    print(f"✅ All 8 TDD phases present and correct (Schema v2.0)")
    ...
    print()
    print("Ready to execute: /nw:execute @software-crafter 'des-us007-boundary-rules' '01-01'")
    print("=" * 80)
