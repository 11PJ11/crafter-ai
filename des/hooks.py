...
class SubagentStopHook:
    ...
    def on_agent_complete(self, step_file_path: str) -> HookResult:
        ...
        # Check for silent completion (all phases NOT_EXECUTED)
        if self._detect_silent_completion(phase_log):
    ...
    def _detect_silent_completion(self, phase_log: List[dict]) -> bool:
        ...
        Returns:
            True if all phases are NOT_EXECUTED, False otherwise
        """
        return all(phase.get("status") == "NOT_EXECUTED" for phase in phase_log)

    def _count_not_executed_phases(self, phase_log: List[dict]) -> int:
        """Count how many phases have NOT_EXECUTED status.
        ...
        """
        return sum(1 for phase in phase_log if phase.get("status") == "NOT_EXECUTED")
    ...
    def _populate_silent_completion_failure(self, result: HookResult, not_executed_count: int) -> None:
        ...
        Args:
            result: HookResult object to populate
            not_executed_count: Number of phases that were NOT_EXECUTED
        """
        result.validation_status = "FAILED"
        ...
        result.recovery_suggestions = [
            "Review agent transcript to determine why no phases were executed.",
            "Check for early errors or exceptions that prevented phase execution.",
            "Re-run the step to ensure proper execution: `/nw:execute @software-crafter <step-file>`"
        ]
