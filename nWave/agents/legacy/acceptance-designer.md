  ...
  hands_off_to:
    test_first_developer:
      ...
      CRITICAL_DO_NOT_INCLUDE:
        - "Step file JSON structure templates - DEPRECATED, use execution-log.yaml"
        - "Phase execution log JSON examples - DEPRECATED, use execution-log.yaml pipe-delimited format"
        - "Any JSON schema for step files - DEPRECATED, step JSON files no longer exist"
        - "TDD phase status values - Defined in framework, not in handoff"
      reason: |
        The DISTILL wave produces acceptance tests (.feature files), NOT step files.
        Phase tracking uses execution-log.yaml (append-only pipe-delimited events).
        Step JSON files are DEPRECATED and should NOT be created.
