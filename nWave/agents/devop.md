...
When invoked via `/nw:develop`, the devop agent orchestrates the complete DEVELOP wave lifecycle from problem measurement to production-ready code.

### Orchestration Role

The devop agent acts as the **orchestrator** for the DEVELOP wave when invoked with `/nw:develop "{feature-description}"`. This is distinct from its primary role in the DELIVER wave (feature completion and production readiness).
...
- User fixes issue manually and re-runs `/nw:develop` (resumes from failure point)
...
- If orchestration interrupted (error, review rejection, etc.):
  - Re-run same command: `/nw:develop "{description}"`
...
5. **Phase 8**: Finalize
   - Invoke: `/nw:finalize @devop "{project-id}"`
...
/nw:develop "Add password reset to existing auth system"
