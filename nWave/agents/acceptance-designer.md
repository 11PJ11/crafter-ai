  ...
  hands_off_to:
    test_first_developer:
      ...
      CRITICAL_DO_NOT_INCLUDE:
        ...
        - "Any JSON schema for step files - Single Source of Truth is step-tdd-cycle-schema.json"
        - "TDD phase status values - Defined in framework, not in handoff"
      reason: |
        The DISTILL wave produces acceptance tests (.feature files), NOT step JSON files.
