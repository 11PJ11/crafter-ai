...
- User provides: `/nw:finalize @nw-devop "auth-upgrade"`
- Extract agent name: `nw-devop` (remove @ prefix)
...
- Input: `/nw:finalize  @nw-devop  "auth-upgrade"`
- After parsing:
  - agent_name = "nw-devop" (whitespace trimmed)
  - project_id = "auth-upgrade" (quotes removed)
- Input: `/nw:finalize @nw-devop "auth-upgrade" extra`
...
**For nw-devop finalizing auth-upgrade project**:
```
Task: "You are the nw-devop agent.
...
**For nw-solution-architect finalizing microservices project**:
```
...
/nw:finalize @nw-data-engineer "analytics-pipeline"
```
