...
> ⚠️ **IMPORTANT**: This command is executed by the **main Claude instance**, NOT by a specialized agent. The main instance orchestrates the workflow by delegating to specialized agents (nw-researcher, nw-software-crafter, nw-solution-architect, etc.) via the Task tool.
...
Task(
    subagent_type="nw-researcher",
    prompt=f'''
You are a nw-researcher agent creating a measurement baseline.
...
| `/nw:roadmap` | `nWave/tasks/nw/roadmap.md` | nw-solution-architect |
...
3. ✅ **YOU** invoke sub-commands using appropriate agents:
   - For implementation: Use `Task` tool with `@nw-software-crafter`
...
Task(
  subagent_type="nw-researcher",  # NOT nw-software-crafter - per baseline.md spec
...
Task(
  ...
  prompt=BOUNDARY_TEMPLATE.format(
      ...
      actual_command='/nw:execute @nw-software-crafter "{step_id}"'
  ),
...
Task(
  ...
  prompt=BOUNDARY_TEMPLATE.format(
      ...
      actual_command='/nw:review @nw-software-crafter baseline "{artifact-path}"'
  ),
...
Task(
  ...
  prompt=BOUNDARY_TEMPLATE.format(
      ...
      actual_command='/nw:roadmap @nw-solution-architect "{feature_description}"'
  ),
...
1. **Execute review with retry loop** (embedded script):
   ...
   approved, attempts, final_status, rejection_reasons = execute_review_with_retry(
       reviewer_agent='@nw-software-crafter',
...
You are a nw-solution-architect agent creating an implementation roadmap.
  ...
  - phase_id: "01"
    ...
    steps:
      - step_id: "01-01"
        ...
        suggested_agent: <nw-researcher|nw-software-crafter|etc>
...
2. **Evaluate findings and act**:
   ```python
   # After nw-solution-architect creates roadmap.yaml:
...
2. **Execute each step in order**:
   ...
   for i, step in enumerate(sorted_steps, 1):
       ...
       print(f"Invoking: Task tool with @nw-software-crafter for step {step_id}")
       ...
       execute_skill(agent="nw-software-crafter", project_id=project_id, step_id=step_id)
...
1. **[FOREGROUND]** Architecture refactoring (L4-L6) via @nw-software-crafter
...
```python
# Invoke @nw-software-crafter for architecture refactoring
...
You are a nw-devop agent finalizing and archiving a completed feature.
...
```bash
/nw:execute @nw-software-crafter "order-management" "01-02"
```
...
/nw:execute @nw-software-crafter "project-id" "01-01"  # ✅ NEW (Schema v2.0)
/nw:execute @nw-software-crafter "project-id" "01-02"  # ✅ NEW (Schema v2.0)
/nw:execute @nw-software-crafter "project-id" "01-03"  # ✅ NEW (Schema v2.0)
