...
**Wave**: CROSS_WAVE
**Agents**: Nova (researcher) + Quill (documentarist)
**Command**: `*document`
...
1. **Research Review Gate** (Phase 1.5): researcher-reviewer validates research quality before documentation
2. **Documentation Review Gate** (Phase 2.5): documentarist-reviewer validates documentation quality before handoff
...
- `researcher-reviewer` (Scholar persona) - validates research quality, evidence, bias, cross-referencing
- `documentarist-reviewer` (Quill persona) - validates DIVIO compliance, collapse detection, classification accuracy
...
Phase 1.5: Research Review
  ├─> researcher-reviewer (Scholar) performs independent validation
  ├─> If APPROVED → proceed to Phase 2
  ├─> If NEEDS_REVISION → iterate (max 2 cycles)
  └─> If REJECTED → escalate to user

Phase 2: Documentation
  ├─> documentarist (Quill) produces documentation
  │
Phase 2.5: Documentation Review
  ├─> documentarist-reviewer (Quill) performs independent validation
...
5. **Orchestrate review workflow**:
   - Invoke researcher-reviewer after research phase completion
   - Handle review feedback iteration (max 2 cycles)
   - Invoke documentarist-reviewer after documentation phase completion
   - Handle review feedback iteration (max 2 cycles)
   - Escalate to user if iteration limits exceeded or critical failures

### Agent Prompt Must Contain

**Phase 1: Research (Nova @researcher)**
...
**Phase 1.5: Research Review (Scholar @researcher-reviewer)**
...
**Phase 2: Documentation (Quill @documentarist)**
...
**Phase 2.5: Documentation Review (Quill @documentarist-reviewer)**
...
- nWave/agents/researcher.md - Research agent DIVIO framework knowledge (for extraction)
- nWave/agents/documentarist.md - Documentation agent validation rules (for extraction)
...
**Enhanced with Layer 4 Peer Review**: This workflow integrates peer review at two critical points - after research completion (researcher-reviewer validates research quality) and after documentation completion (documentarist-reviewer validates documentation quality). Reviews are mandatory quality gates that must pass before proceeding to the next phase.
...
### Step 2: Phase 1 - Research (Nova @researcher)
...
### Step 3: Phase 1.5 - Research Review (Scholar @researcher-reviewer)

**Orchestrator invokes researcher-reviewer for quality assurance**:

**CRITICAL**: Use Task tool to invoke researcher-reviewer agent (NOT /nw:review command). Sub-agents cannot execute skills.

```
Task: "You are the researcher-reviewer agent acting as an expert reviewer.
...
### Step 4: Phase 2 - Documentation (Quill @documentarist)

**Orchestrator invokes documentarist with COMPLETE INLINE PROMPT**:
...
[COMPLETE DIVIO FRAMEWORK FROM documentarist.md]

DOCUMENTATION TYPE: {type}

TYPE-SPECIFIC REQUIREMENTS:
[TYPE-SPECIFIC VALIDATION RULES FROM documentarist.md]

COLLAPSE PREVENTION:
[COMPLETE ANTI-PATTERNS FROM documentarist.md]
...
### Step 5: Phase 2.5 - Documentation Review (Quill @documentarist-reviewer)

**Orchestrator invokes documentarist-reviewer for quality assurance**:

**CRITICAL**: Use Task tool to invoke documentarist-reviewer agent (NOT /nw:review command). Sub-agents cannot execute skills.

```
Task: "You are the documentarist-reviewer agent acting as an expert reviewer.
...
```yaml
reviews:
  - reviewer: "documentarist-reviewer"
    date: "{ISO-8601-timestamp}"
...
2. **If Review NEEDS_REVISION**:
   - Extract critique issues from review metadata
   - Re-invoke documentarist (Quill) with feedback:
     ```
     "Address review feedback from documentarist-reviewer:
     {list of critiques with severity and recommendations}

     Update documentation to resolve these issues."
     ```
   - Re-invoke documentarist-reviewer for validation
   - Maximum 2 iteration cycles
   - If still failing after 2 cycles: Escalate to user
...
3. **If Review RESTRUCTURE_REQUIRED**:
   ...
   - If user chooses split: Re-invoke documentarist with split instructions
...
### Research Review Phase (Scholar @researcher-reviewer)

- [ ] Research passes peer review (researcher-reviewer approval)
...
### Documentation Review Phase (Quill @documentarist-reviewer)

- [ ] Documentation passes peer review (documentarist-reviewer approval)
...
Action: Re-invoking documentarist with feedback (Iteration {X}/2)
```
...
✅ Agent orchestration (researcher → researcher-reviewer → documentarist → documentarist-reviewer)
