...
def update_phase_with_tests(
    ...
    print(
        f"Updated phase {phase_index}: {phase['phase_name']} -> {status}/{outcome} ({passed}/{total} tests passed)"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 10:
        print(
            "Usage: update_phase_with_tests.py <step_file> <phase_index> <status> <outcome> <outcome_details> <total> <passed> <failed> <skipped> [notes]"
        )
        sys.exit(1)
