...
class RealSubagentStopHook(HookPort):
    ...
    Validates step file state after sub-agent completion by checking for:
    - Abandoned phases (IN_PROGRESS status)
    - Silent completion (all phases NOT_EXECUTED)
    - Incomplete phases (EXECUTED without outcome)
    - Invalid skips (SKIPPED without blocked_by reason)
    - Turn limit exceeded
    - Timeout exceeded
