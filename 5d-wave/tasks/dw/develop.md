...
**Command**: `/dw:develop "{feature-description}"`
...
- **Single Command**: One command executes entire wave (`/dw:develop "{description}"`)
...
**⚠️ BREAKING CHANGE**: This is a complete redesign of the `/dw:develop` command.

**OLD Signature** (DEPRECATED):
```bash
/dw:develop {feature-name} --step {step-id}
```

**NEW Signature**:
```bash
/dw:develop "{feature-description}"
```

**Migration**: For single-step execution, use `/dw:execute` instead (see Migration Guide below).
...
3. **Validation**:
   ...
   if feature_description.strip().startswith("--"):
       ERROR: "Invalid syntax. Use: /dw:develop \"{description}\" (not --step flag)"
       HINT: "For single step execution, use /dw:execute instead"
       EXIT
   ```
...
2. **If baseline creation needed** (status == 'missing'):
   ...
   ```bash
   /dw:baseline "{feature_description}"
   ```
...
1. **Execute review with retry loop** (embedded script):
   ...
   if not approved:
       ...
       print("3. Re-run: /dw:develop \"{feature_description}\"")
...
2. **If roadmap creation needed**:
   ...
   ```bash
   /dw:roadmap @solution-architect "{feature_description}"
   ```
...
2. **Review each step file with retry**:
   ...
   for i, step_file in enumerate(step_files, 1):
       ...
       approved, attempts, final_status, rejection_reasons = execute_review_with_retry(
           ...
           project_id=project_id,
           max_attempts=2,
           regenerate_command=f'/dw:split @devop "{project_id}" --regenerate-step {step_id}'
       )
...
3. **Execute each step in order**:
   ...
   for i, step_file in enumerate(sorted_step_files, 1):
       ...
       print(f"Invoking: /dw:execute @software-crafter \"{step_file}\"")
...
def execute_review_with_retry(reviewer_agent, artifact_type, artifact_path,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        review_cmd = f'/dw:review {reviewer_agent} {artifact_type.lower().replace(" ", "-")} "{artifact_path}"'
            ...
            if attempt < max_attempts:
                ...
                if regenerate_command:
                    ...
                    if artifact_type == 'Baseline':
                        regen_cmd = f'/dw:baseline "{project_description}" --regenerate --feedback "{rejection_reason}"'
                    elif artifact_type == 'Roadmap':
                        regen_cmd = f'/dw:roadmap @solution-architect "{project_description}" --regenerate --feedback "{rejection_reason}"'
                    else:
...
```bash
/dw:develop "Implement user authentication with JWT tokens and session management"
```
...
### Greenfield Project - Full 5D-WAVE

```bash
# Wave 1: DISCUSS (optional - can skip if requirements clear)
/dw:discuss "User authentication system requirements"

# Wave 2: DESIGN (optional - can skip if architecture defined)
/dw:design "Microservices with hexagonal architecture for auth"
...
/dw:develop "Add password reset functionality to existing auth system"
...
**⚠️ BREAKING CHANGE**: Complete redesign of `/dw:develop` command.

**REMOVED Functionality**:
- ❌ `/dw:develop {feature} --step {id}` syntax
- ❌ Granular single step execution via develop command

**NEW Functionality**:
- ✅ `/dw:develop "{description}"` for complete wave orchestration
...
**NEW** (use `/dw:execute` instead):
```bash
/dw:execute @software-crafter "docs/feature/order-management/steps/01-02.json"
```

**Explanation**: The `/dw:execute` command now provides the complete 11-phase TDD execution for a single step that `/dw:develop --step` used to provide.
...
/dw:roadmap @solution-architect "goal description"
/dw:split @devop "project-id"
/dw:develop feature-name --step 01-01  # ❌ NO LONGER WORKS
/dw:develop feature-name --step 01-02  # ❌ NO LONGER WORKS
/dw:develop feature-name --step 01-03  # ❌ NO LONGER WORKS
/dw:finalize @devop "project-id"
```
...
/dw:roadmap @solution-architect "goal description"
/dw:split @devop "project-id"
/dw:execute @software-crafter "docs/feature/{id}/steps/01-01.json"  # ✅ NEW
/dw:execute @software-crafter "docs/feature/{id}/steps/01-02.json"  # ✅ NEW
/dw:execute @software-crafter "docs/feature/{id}/steps/01-03.json"  # ✅ NEW
/dw:finalize @devop "project-id"
```
...
/dw:baseline "implement shopping cart"
# ... wait ...
/dw:roadmap @solution-architect "implement shopping cart"
# ... wait ...
/dw:split @devop "shopping-cart"
# ... wait ...
/dw:develop shopping-cart --step 01-01
# ... wait ...
/dw:develop shopping-cart --step 01-02
# ... (repeat for all steps) ...
/dw:finalize @devop "shopping-cart"
```

**NEW** (single command):
```bash
/dw:develop "Implement shopping cart with checkout validation"
...
**After DEVELOP completes**: `/dw:demo "{project-id}"` → DEMO wave
...
1. **Breaking Change Rationale**: Perfect semantics over backwards compatibility - `/dw:develop` should develop the COMPLETE feature, not a single step
...
4. **Re-run command**: `/dw:develop "{description}"` (resumes from failure point)
