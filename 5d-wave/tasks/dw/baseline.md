...
- User provides: `/dw:baseline "optimize test execution time"`
...
Usage: /dw:baseline "description of what you want to optimize/improve/build"

Examples:
  /dw:baseline "optimize test execution time"
  /dw:baseline "fix roadmap prioritization process"
  /dw:baseline "add baseline enforcement to workflow"
```
...
Invokes the researcher agent to establish a quantitative or qualitative baseline BEFORE roadmap creation. This command creates the baseline.yaml file required by /dw:roadmap.
...
/dw:baseline "prevent roadmap prioritization errors"
...
**Next Command**:
```bash
/dw:roadmap @{appropriate-agent} "{goal-description}"
```
...
# =============================================================================
#
# This file is REQUIRED before running /dw:roadmap.
...
baseline:
  ...
  requirements_source:  # (REQUIRED for feature_development)
    ...
    requirements:  # (OPTIONAL) List specific requirements
      - "Must block /dw:roadmap without baseline file"
  ...
  simplest_alternatives_considered:
    - alternative: "Add checklist item to /dw:roadmap command"
...
#     scope:
#       in_scope: ["New /dw:baseline command", "Baseline file format"]
#       out_of_scope: ["Changes to /dw:roadmap enforcement"]
#
#   current_state:
